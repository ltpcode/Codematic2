...
namespace Codematic
{
    public partial class CodeTemp : Form
    {
        ...
        private void Run()
        {
            ...
            string tempxslt = @"Template\temp.cmt"; //临时模板文件
            try
            {
            ...
            try
            {
                BuilderTemp bt = new BuilderTemp(dbobj, dbname, tablename,tablename,
                    GetFieldlist(), GetKeyFields(), tempxslt, dbset);
                strcode = bt.GetCode();
            }
            catch (System.Exception ex)
            {
            ...
            mainfrm.AddSinglePage(new CodeEditor(filename, "cs", false), "Class1.cs");
        }
    }
}
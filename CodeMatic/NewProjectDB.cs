...
namespace Codematic
{
    ...
    public partial class NewProjectDB : Form
    {
        ...
        /// <param name="vsVer">VS版本</param>
        public NewProjectDB(string longservername, NewProject fromForm, string proOutPath, string appFrame, string proName, string vsVer)
        {
            InitializeComponent();
        ...
        public void CopyDirectory(string SourceDirectory, string TargetDirectory)
        {
            ...
            for (int i = 0; i < filescount; ++i)
            {
                if ((sourceFiles[i].Extension == ".sln") || (sourceFiles[i].Extension == ".suo"))
                {
                    ...
                    if (VSVer == "2008")
                    {
                        if (File.Exists(sourceFiles[i].FullName))
                        {
                            string tempstr = "";
                            using (StreamReader sr = new StreamReader(sourceFiles[i].FullName, Encoding.Default))
                            {
                                tempstr = sr.ReadToEnd().Replace("Format Version 11.00", "Format Version 10.00").Replace("Visual Studio 2010", "Visual Studio 2008");
                                sr.Close();
                            }
                            WriteFile(target.FullName + "\\" + ProName + sourceFiles[i].Extension, tempstr);
                        }
                    }
                }
                else
                    if ((sourceFiles[i].Extension == ".csproj") && (VSVer == "2008"))
                    {
                        if (File.Exists(sourceFiles[i].FullName))
                        {
                            string tempstr = "";
                            using (StreamReader sr = new StreamReader(sourceFiles[i].FullName, Encoding.Default))
                            {
                                tempstr = sr.ReadToEnd().Replace("ToolsVersion=\"4.0\"", "ToolsVersion=\"3.5\"").Replace("<Import Project=\"$(MSBuildExtensionsPath32)\\Microsoft\\VisualStudio\\v10.0\\WebApplications\\Microsoft.WebApplication.targets\" />","");
                                sr.Close();
                            }
                            WriteFile(target.FullName + "\\" + sourceFiles[i].Name, tempstr);
                        }

                    }
                    else
                        if ((sourceFiles[i].Extension == ".config") && (VSVer == "2008"))
                        {
                            if (File.Exists(sourceFiles[i].FullName))
                            {
                                string tempstr = "";
                                using (StreamReader sr = new StreamReader(sourceFiles[i].FullName, Encoding.Default))
                                {
                                    tempstr = sr.ReadToEnd().Replace("targetFramework=\"4.0\"", "").Replace("<pages controlRenderingCompatibilityVersion=\"3.5\" clientIDMode=\"AutoID\"/>", " ");
                                    sr.Close();
                                }
                                //WriteFile(target.FullName + "\\" + sourceFiles[i].Name, tempstr);
                                StreamWriter sw = new StreamWriter(target.FullName + "\\" + sourceFiles[i].Name, false, Encoding.UTF8);//,false);
                                sw.Write(tempstr);
                                sw.Flush();
                                sw.Close();
                            }
        ...
        private void CreatCsS3()
        {
            ...
            if (File.Exists(tempaspx))
            {
                ...
                else
                {
                    ...
                    if (keyField.Length == 0) //如果多主键中，无自动增长列，则用多主键循环列
                    {
            ...
            if (File.Exists(tempaspxcs))
            {
                using (StreamReader sr = new StreamReader(tempaspxcs, Encoding.Default))
                {
                    ...
                    {
                        tempstr = tempstr.Replace("bll.DeleteList(idlist)", "#warning 代码生成警告：多主键情况无法生成批量删除，请手工修改代码. //bll.DeleteList(idlist)");
                    }
                    sr.Close();
                }
                WriteFile(fileaspxcs, tempstr);
        ...
        private void ReplaceNamespaceProj(string filename, string spacename)
        {
            ...
            text = text.Replace(">Maticsoft.", ">" + spacename + ".");
